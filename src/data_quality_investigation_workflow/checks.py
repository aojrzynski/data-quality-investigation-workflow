...
This module runs route-specific checks after the issue has been classified for
planning. The checks produce aggregate review signals such as duplicate counts,
null percentages, date range summaries, schema counts, and current-vs-baseline
deltas. Signal labels such as ``present`` or ``absent`` are cues for human
review, not verdicts that the reported issue is true or false.

The checks avoid raw records, row numbers, category labels, duplicated values,
and example values. Where a route needs an assumption, such as daily cadence for
date-gap checks, the assumption is written into limitations.
"""
...
# Duplicate-key checks count repeated candidate-key values and key nulls.
# They never write the duplicated values themselves.
def _duplicate_key_items(
    loaded_dataset: LoadedDataset, dataset_profile: dict[str, Any], plan: dict[str, Any]
) -> list[dict[str, Any]]:
    columns = _candidate_names(plan) or _name_matches(
        loaded_dataset.column_names, KEY_FALLBACK_TERMS
    )
    ...
    for name in columns:
        ...
        duplicate_count = max(
            int(non_null.shape[0]) - int(non_null.nunique(dropna=True)), 0
        )
    ...
    return [
        {
            ...
            "summary": "At least one candidate key column has duplicate non-null values."
            if duplicate_columns
            else "No duplicate non-null values were detected in candidate key columns.",
            ...
            "limitations": _standard_limitations(
                "Duplicate values are counted as aggregates only."
            ),
        },
        {
            ...
            "summary": "At least one candidate key column contains null values."
            if null_columns
            else "No null values were detected in candidate key columns.",
            ...
            "limitations": _standard_limitations(
                "Null values are counted as aggregates only."
            ),
        },
    ]


# Null checks summarize the current file. Baseline evidence, when available,
# is added separately from the baseline comparison artifact.
def _null_items(
    loaded_dataset: LoadedDataset, _profile: dict[str, Any], plan: dict[str, Any]
) -> list[dict[str, Any]]:
    ...
    return [
        {
            ...
            "summary": "The current dataset contains null values in assessed columns. A baseline is required for current-vs-baseline increase evidence."
            if total_nulls
            else "No null values were detected in assessed columns. A baseline is required for current-vs-baseline increase evidence.",
            "related_columns": [
                name for name in columns if _null_count(loaded_dataset, name) > 0
            ],
            "metrics": {
            ...
            "limitations": _standard_limitations(
                "No baseline comparison was supplied for this current-dataset-only evidence item."
            ),
        }
    ]


# Date-gap checks use a daily-cadence assumption. They count missing
# periods and ranges but do not write the missing dates.
def _date_items(
    loaded_dataset: LoadedDataset, profile: dict[str, Any], plan: dict[str, Any]
) -> list[dict[str, Any]]:
    columns = (
        _candidate_names(plan)
        or _profile_kind_names(profile, {"datetime"})
        or _name_matches(loaded_dataset.column_names, DATE_FALLBACK_TERMS)
    )
    parsed = []
    for name in columns:
        series = _series(loaded_dataset, name)
        ...
        if not dates.empty:
            parsed.append(
                (
                    name,
                    dates.dt.normalize() if hasattr(dates.dt, "normalize") else dates,
                )
            )
    ...
    return [
        {
            ...
            "signal_strength": "low",
            "summary": "Date ranges were summarized for parsed candidate date columns."
            if parsed
            else "No parseable candidate date columns were available for date range evidence.",
            "related_columns": [name for name, _dates in parsed],
            "metrics": metrics,
            "limitations": _standard_limitations(
                "Dates are recorded only as aggregate range evidence."
            ),
        },
        {
            ...
            "summary": "A daily-cadence aggregate gap signal is present. Missing date lists are not written."
            if missing_total
            else "No daily-cadence aggregate gap was detected in parsed candidate date columns.",
            "related_columns": [name for name, _dates in parsed],
            "metrics": metrics,
            "limitations": _standard_limitations(
                "Daily cadence is assumed only for planning evidence."
            ),
        },
    ]


# Total-change checks record numeric totals and row counts as aggregates.
# A baseline is needed before these become change signals.
def _total_items(
    loaded_dataset: LoadedDataset, profile: dict[str, Any], plan: dict[str, Any]
) -> list[dict[str, Any]]:
    columns = [
        ...
        for name in (
            _candidate_names(plan)
            or _profile_kind_names(profile, {"integer", "decimal"})
        )
        if _is_numeric(loaded_dataset, name)
    ]
    totals = []
    for name in columns:
        series = _series(loaded_dataset, name)
...
# Schema checks describe column shape and inferred-kind counts. They do
# not decide whether a schema change is acceptable.
def _schema_items(
    _loaded_dataset: LoadedDataset, profile: dict[str, Any], plan: dict[str, Any]
) -> list[dict[str, Any]]:
    ...
    return [
        {
            ...
            "metrics": {
                ...
                "duplicate_column_name_count": len(
                    dataset.get("duplicate_column_names", [])
                ),
...
# Category checks use cardinality shape only. Category labels and
# distributions are excluded from artifacts.
def _category_items(
    loaded_dataset: LoadedDataset, profile: dict[str, Any], plan: dict[str, Any]
) -> list[dict[str, Any]]:
    columns = _candidate_names(plan) or _likely_category_names(
        profile, loaded_dataset.row_count
    )
    unique_counts = []
    for name in columns:
        series = _series(loaded_dataset, name)
    ...
    return [
        {
            ...
            "metrics": {
                ...
                "any_candidate_categorical_column_has_high_cardinality": max_unique_percentage
                > 50.0,
            },
            "limitations": _standard_limitations(
                "Category labels and distributions are not written."
            ),
        }
    ]


# General checks provide a broad aggregate summary when the issue does
# not match a more specific route.
def _general_items(
    _loaded_dataset: LoadedDataset, profile: dict[str, Any], _plan: dict[str, Any]
) -> list[dict[str, Any]]:
    dataset = profile.get("dataset", {})
    columns = profile.get("columns", [])
    kinds = Counter(
        str(column.get("inferred_kind", "mixed_or_unknown")) for column in columns
    )
    total_nulls = sum(int(column.get("null_count", 0)) for column in columns)
    columns_with_nulls = sum(
        1 for column in columns if int(column.get("null_count", 0)) > 0
    )
    return [
        {
            ...
            "metrics": {
                ...
                "total_null_cells": total_nulls,
                "columns_with_nulls": columns_with_nulls,
                "numeric_column_count": kinds.get("integer", 0)
                + kinds.get("decimal", 0),
            ...
            "limitations": _standard_limitations(
                "This check records profile metadata only."
            ),
        }
    ]


# Baseline checks convert current-vs-baseline aggregate comparison
# sections into evidence items for the selected route.
def _baseline_items(
    route_name: str, comparison: dict[str, Any]
) -> list[dict[str, Any]]:
    builders = {
    ...
    return [
        {
            ...
            "status": "executed",
            "signal": "present"
            if duplicate.get("columns_with_higher_duplicate_count", 0)
            else "absent",
            "signal_strength": "medium"
            if duplicate.get("columns_with_higher_duplicate_count", 0)
            else "low",
            "summary": "Duplicate aggregate comparison signal present; current candidate-key duplicate aggregates are higher than baseline for at least one column."
            if duplicate.get("columns_with_higher_duplicate_count", 0)
            else "Duplicate aggregate comparison signal was recorded; current candidate-key duplicate aggregates are not higher than baseline.",
            "related_columns": _delta_columns(duplicate),
            "metrics": {
                "candidate_columns_compared": int(
                    duplicate.get("candidate_columns_compared", 0)
                ),
                "columns_with_higher_duplicate_count": int(
                    duplicate.get("columns_with_higher_duplicate_count", 0)
                ),
                "max_duplicate_count_delta": int(
                    duplicate.get("max_duplicate_count_delta", 0)
                ),
    ...
    return [
        {
            ...
            "status": "executed",
            "signal": "present"
            if nulls.get("columns_with_higher_null_percentage", 0)
            else "absent",
            "signal_strength": "medium"
            if nulls.get("columns_with_higher_null_percentage", 0)
            else "low",
            "summary": "Null aggregate comparison signal present; current null percentage is higher than baseline for at least one assessed column."
            if nulls.get("columns_with_higher_null_percentage", 0)
            else "Null aggregate comparison signal was recorded; current null percentages are not higher than baseline for assessed columns.",
            "related_columns": _delta_columns(nulls),
            "metrics": {
                "columns_compared": int(nulls.get("columns_compared", 0)),
                "columns_with_higher_null_percentage": int(
                    nulls.get("columns_with_higher_null_percentage", 0)
                ),
                "columns_with_lower_null_percentage": int(
                    nulls.get("columns_with_lower_null_percentage", 0)
                ),
                "max_null_percentage_delta": float(
                    nulls.get("max_null_percentage_delta", 0.0)
                ),
                "column_deltas": nulls.get("column_deltas", []),
            },
            "limitations": _baseline_limitations(
                "This comparison does not claim final issue confirmation."
            ),
        }
    ]
    ...
    return [
        {
            ...
            "status": "executed",
            "signal": "present"
            if dates.get("columns_with_missing_daily_period_delta", 0)
            else "absent",
            "signal_strength": "medium"
            if dates.get("columns_with_missing_daily_period_delta", 0)
            else "low",
            ...
            "metrics": {
                ...
                "columns_with_range_change": int(
                    dates.get("columns_with_range_change", 0)
                ),
                "columns_with_missing_daily_period_delta": int(
                    dates.get("columns_with_missing_daily_period_delta", 0)
                ),
                "daily_cadence_assumption": bool(
                    dates.get("daily_cadence_assumption", True)
                ),
                "column_deltas": dates.get("column_deltas", []),
            },
            "limitations": _baseline_limitations(
                "Daily cadence is assumed; missing date lists are not written."
            ),
        }
    ]
    ...
    return [
        {
            ...
            "status": "executed",
            "signal": "present"
            if numeric.get("columns_with_total_delta", 0) or rows.get("delta", 0)
            else "absent",
            "signal_strength": "medium"
            if numeric.get("columns_with_total_delta", 0) or rows.get("delta", 0)
            else "low",
            ...
            "metrics": {
                ...
                "columns_with_total_delta": int(
                    numeric.get("columns_with_total_delta", 0)
                ),
                "max_absolute_total_delta": float(
                    numeric.get("max_absolute_total_delta", 0.0)
                ),
                "column_deltas": numeric.get("column_deltas", []),
            },
            "limitations": _baseline_limitations(
                "Numeric totals are aggregate sums only."
            ),
        }
    ]
    ...
    return [
        {
            ...
            "status": "executed",
            "signal": "present"
            if schema.get("added_columns")
            or schema.get("removed_columns")
            or schema.get("inferred_kind_change_count", 0)
            else "absent",
            "signal_strength": "medium"
            if schema.get("added_columns") or schema.get("removed_columns")
            else "low",
            "summary": "Schema aggregate comparison signal was recorded using column names and inferred-kind metadata only.",
            "related_columns": list(schema.get("added_columns", []))
            + list(schema.get("removed_columns", [])),
            "metrics": {
                ...
                "inferred_kind_change_count": int(
                    schema.get("inferred_kind_change_count", 0)
                ),
                "inferred_kind_changes": schema.get("inferred_kind_changes", []),
            },
            "limitations": _baseline_limitations(
                "Schema comparison does not claim root cause."
            ),
        }
    ]
    ...
    return [
        {
            ...
            "status": "executed",
            "signal": "present"
            if category.get("columns_with_unique_count_delta", 0)
            else "absent",
            "signal_strength": "medium"
            if category.get("columns_with_unique_count_delta", 0)
            else "low",
            ...
            "metrics": {
                ...
                "columns_with_unique_count_delta": int(
                    category.get("columns_with_unique_count_delta", 0)
                ),
                "max_unique_count_delta": int(
                    category.get("max_unique_count_delta", 0)
                ),
                "columns_with_high_cardinality_signal_delta": int(
                    category.get("columns_with_high_cardinality_signal_delta", 0)
                ),
                "column_deltas": category.get("column_deltas", []),
            },
            "limitations": _baseline_limitations(
                "Category labels and full category distributions are not written."
            ),
        }
    ]
    ...
    return [
        {
            ...
            "metrics": general,
            "limitations": _baseline_limitations(
                "General comparison is not route-specific final evidence."
            ),
        }
    ]
...
def _delta_columns(section: dict[str, Any]) -> list[str]:
    return [
        str(delta.get("column_name"))
        for delta in section.get("column_deltas", [])
        if delta.get("column_name")
    ]
...
def _candidate_names(plan: dict[str, Any]) -> list[str]:
    return [
        str(column.get("name"))
        for column in plan.get("dataset_context", {}).get("candidate_columns", [])
        if column.get("name")
    ]
...
def _profile_kind_names(profile: dict[str, Any], kinds: set[str]) -> list[str]:
    return [
        str(column.get("name"))
        for column in profile.get("columns", [])
        if str(column.get("inferred_kind")) in kinds
    ]
...
def _likely_category_names(profile: dict[str, Any], row_count: int) -> list[str]:
    ...
    for column in profile.get("columns", []):
        ...
        if any(term in name.casefold() for term in CATEGORY_FALLBACK_TERMS) or (
            kind == "text" and _percentage(unique_count, row_count) <= 50.0
        ):
            names.append(name)
    return names
...
def _with_ids(items: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return [
        {"evidence_id": f"ev-{index:03d}", **item}
